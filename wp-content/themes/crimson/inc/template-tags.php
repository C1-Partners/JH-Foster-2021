...
function crimson_favicons(){
	...
	$standardSizes = array(
		...
		32, // taskbar shortcut
		96, // desktop shortcut
		192 // largest size needed by google
	);
	$touchIcons = array(
		120, // iPhone Retina
		152, // iPad Retina
	...
	$themeColor = '#ffffff';
	$iconPath = get_template_directory_uri() . '/assets/images/icons/';
	?>
	<?php foreach ($standardSizes as $size) { ?>
		<link rel="icon" type="image/png" sizes="<?php echo $size . 'x' . $size ?>" href="<?= $iconPath . 'favicon-'. $size . '.png' ?>">
	<?php } ?>  
	<?php foreach ($touchIcons as $size) { ?>
		<link rel="apple-touch-icon" sizes="<?php echo $size . 'x' . $size ?>" href="<?= $iconPath . 'favicon-'. $size . '.png' ?>">
	<?php } ?>
	<?php foreach ($msTiles as $size) { ?>
		<meta name="msapplication-square<?php echo $size . 'x' . $size ?>logo" content="<?= $iconPath . 'favicon-'. $size . '.png' ?>">
	<?php } ?>
...
function crimson_get_site_logo(){
	$tag = 'a';
...
function crimson_posted_on() {
  ...
  $posted_on = sprintf(
    ...
    esc_html_x( 'Posted on %s', 'post date', 'crimson' ),
 ...
 */
function crimson_posted_by() {
  $byline = sprintf(
    /* translators: %s: post author. */
    esc_html_x( 'by %s', 'post author', 'crimson' ),
 ...
 */
function crimson_entry_footer() {
  // Hide category and tag text for pages.
  if ( 'post' === get_post_type() ) {
    /* translators: used between list items, there is a space after the comma */
    $categories_list = get_the_category_list( esc_html__( ', ', 'crimson' ) );
    if ( $categories_list ) {
      /* translators: 1: list of categories. */
      printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'crimson' ) . '</span>', $categories_list ); // WPCS: XSS OK.
    }

    /* translators: used between list items, there is a space after the comma */
    $tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'crimson' ) );
    if ( $tags_list ) {
      /* translators: 1: list of tags. */
      printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'crimson' ) . '</span>', $tags_list ); // WPCS: XSS OK.
    }
  }
    ...
    comments_popup_link(
      sprintf(
        wp_kses(
          ...
          __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'crimson' ),
          array(
            'span' => array(
              'class' => array(),
            ),
  ...
  edit_post_link(
    sprintf(
      wp_kses(
        ...
        __( 'Edit <span class="screen-reader-text">%s</span>', 'crimson' ),
        array(
          'span' => array(
            'class' => array(),
          ),
 ...
 */
function crimson_post_thumbnail() {
...
*/ 
function crimson_make_title($title, $microTitle = false){
 ...
 */
function crimson_get_inlined($asset){
	switch ($asset) {
		case 'logo':
			$output = '';
			break;
 ...
 */
function crimson_get_icon($asset, $text){
	return crimson_get_inlined($asset) . '<span class="sr-only">' . $text . '</span>';
}
 ...
 */
function crimson_generate_fallback_image($size = 'default', $color = 'random')
{
    $sizesAvailable = [
        'hero',       // 1500 / 600
        'blog-intro',  // 750 / 450
