...
$defaults = [
  "content" => [
		"src" => get_template_directory_uri()."/images/temp/overlay.jpg",
		"alt" => "",
		"text" => "",
	],
  "style" => [
    "type" => "", // if set, to "background-pattern", "graphic-overlay" or "thumbnail"
    "class" => '',
    "id" => '',
    "attrs" => []
  ]
...
gsc_define("img", $defaults, function($data) {
	...
	$text = '<p class="figure__text">' . $data["content"]["text"] . '</p>';
	...
	if ($type == "text-overlay") {
		$class = "figure figure--text-ovrly " . $class;
	}
  ...
  if (!empty($data["style"]["attrs"])) {
	...
	if (empty($type) || $type == "thumbnail") {
		...
		if ($type == "text-overlay") {
			$content = "<figure $id_attr $misc_attrs class='{$class}'>" . $content . $text . "</figure>";
		}
...
gsc_test("img", "", function() {
  echo gsc("img", []);
	...
	echo gsc("img", [
    "content" => [
			"src" => get_template_directory_uri()."/images/temp/placeholder.png",
			"alt" => "test"
		],
		"style" => [
			"type" => "thumbnail"
