...
class Utils {
    ...
    public function acf_file_repeater($files) {
        $html = '';
        $fileSVG = inline_icon('file.svg');
        if ($files) {
            foreach($files as $file) {
                $filename = $file['doc_file']["filename"];
                $file_url = $file['doc_file']["url"];
                $html .= '<p><a href="'.$file_url.'" target="_blank">'.$fileSVG.$filename.'</a></p>';
            }
        }
        return $html; 
    }
